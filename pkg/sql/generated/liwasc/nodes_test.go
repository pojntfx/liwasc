...
func testNodeToOneScanUsingScan(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err := randomize.Struct(seed, &foreign, scanDBTypes, false, scanColumnsWithDefault...); err != nil {
	...
	local.ScanID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.Scan().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadScan(ctx, tx, false, (*[]*Node)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Scan == nil {
	...
	local.R.Scan = nil
	if err = local.L.LoadScan(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Scan == nil {
		t.Error("struct should have been eager loaded")
...
func testNodeToOneSetOpScanUsingScan(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, scanDBTypes, false, strmangle.SetComplement(scanPrimaryKeyColumns, scanColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, scanDBTypes, false, strmangle.SetComplement(scanPrimaryKeyColumns, scanColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*Scan{&b, &c} {
		err = a.SetScan(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.ScanID != x.ID {
			t.Error("foreign key was wrong value", a.ScanID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.ScanID))
		reflect.Indirect(reflect.ValueOf(&a.ScanID)).Set(zero)
		...
		if a.ScanID != x.ID {
			t.Error("foreign key was wrong value", a.ScanID, x.ID)
		}
	}
}
