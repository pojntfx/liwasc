...
var NodeScanWhere = struct {
	...
	NodeID:    whereHelperint64{field: "\"node_scans\".\"node_id\""},
}
...
var (
	nodeScanAllColumns            = []string{"id", "created_at", "done", "node_id"}
	nodeScanColumnsWithoutDefault = []string{"created_at", "done", "node_id"}
...
func (o *NodeScan) Node(mods ...qm.QueryMod) nodeQuery {
...
func (o *NodeScan) Services(mods ...qm.QueryMod) serviceQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"services\".\"node_scan_id\"=?", o.ID),
	)
...
func (nodeScanL) LoadNode(ctx context.Context, e boil.ContextExecutor, singular bool, maybeNodeScan interface{}, mods queries.Applicator) error {
	var slice []*NodeScan
	var object *NodeScan
	...
	query := NewQuery(qm.From(`nodes`), qm.WhereIn(`nodes.id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		...
		foreign.R.NodeScans = append(foreign.R.NodeScans, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.NodeID == foreign.ID {
				...
				foreign.R.NodeScans = append(foreign.R.NodeScans, local)
				break
			}
		}
	}
...
func (nodeScanL) LoadServices(ctx context.Context, e boil.ContextExecutor, singular bool, maybeNodeScan interface{}, mods queries.Applicator) error {
	var slice []*NodeScan
	var object *NodeScan
	...
	query := NewQuery(qm.From(`services`), qm.WhereIn(`services.node_scan_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// SetNode of the nodeScan to the related item.
// Sets o.R.Node to related.
// Adds o to related.R.NodeScans.
func (o *NodeScan) SetNode(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Node) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &nodeR{
			NodeScans: NodeScanSlice{o},
		}
	} else {
		related.R.NodeScans = append(related.R.NodeScans, o)
	}
...
func (o *NodeScan) AddServices(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Service) error {
	var err error
