...
func testNetworkScanToManyNodes(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, networkScanDBTypes, true, networkScanColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize NetworkScan struct: %s", err)
	}
	...
	b.NetworkScanID = a.ID
	c.NetworkScanID = a.ID
	...
	for _, v := range check {
		if v.NetworkScanID == b.NetworkScanID {
			bFound = true
		}
		if v.NetworkScanID == c.NetworkScanID {
			cFound = true
		}
	}
	...
	slice := NetworkScanSlice{&a}
	if err = a.L.LoadNodes(ctx, tx, false, (*[]*NetworkScan)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testNetworkScanToManyAddOpNodes(t *testing.T) {
	var err error
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, networkScanDBTypes, false, strmangle.SetComplement(networkScanPrimaryKeyColumns, networkScanColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.ID != first.NetworkScanID {
			t.Error("foreign key was wrong value", a.ID, first.NetworkScanID)
		}
		if a.ID != second.NetworkScanID {
			t.Error("foreign key was wrong value", a.ID, second.NetworkScanID)
		}

		if first.R.NetworkScan != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.NetworkScan != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
